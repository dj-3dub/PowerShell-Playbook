function Get-EotExchangeHygiene {
    ...
    begin {
        ...
        Write-EotLog -Level Info -Message "EXO hygiene start" -Data @{ Env=$Environment }
    }
    process {
        $rows = $null
        $usedMock = $false
        try {
            ...
            $usedMock = $true
            Write-EotLog -Level Warn -Message "Falling back to mock EXO data" -Data @{ Reason = $_.Exception.Message }
            $rows = @(
        ...
        Write-EotLog -Level Info -Message "EXO hygiene complete" -Data @{ Env=$Environment }
    }
}
